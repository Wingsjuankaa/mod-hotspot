...
public:
    ...
    void LoadFromDB()
    {
        ...
        QueryResult result = WorldDatabase.Query("SELECT map_id, x, y, z, radius, xp_multiplier, respawn_multiplier, type FROM mod_hotspot WHERE active = 1");
        if (!result) return;
    ...
    const HotSpotData* GetHotSpotAt(uint32 mapId, float x, float y, float z, uint8 type = 0) const
    {
        for (const auto& spot : hotspots)
            if (spot.map_id == mapId && (type == 0 || spot.type == type))
            {
                float dx = spot.x - x, dy = spot.y - y, dz = spot.z - z;
                if ((dx*dx + dy*dy + dz*dz) <= spot.radius * spot.radius) return &spot;
            }
        return nullptr;
    }
...
public:
    ...
    static bool HandleHotSpotAddCommand(ChatHandler* handler, uint8 type, float radius)
    {
        ...
        handler->PSendSysMessage("¡Hot Spot creado con éxito!");
        return true;
    }
    ...
    static bool HandleHotSpotDeleteCommand(ChatHandler* handler)
    {
        ...
        WorldDatabase.DirectExecute(Acore::StringFormat(
            "DELETE FROM mod_hotspot WHERE map_id = {} AND x > {} - 50 AND x < {} + 50 AND y > {} - 50 AND y < {} + 50",
            player->GetMapId(), player->GetPositionX(), player->GetPositionX(), player->GetPositionY(), player->GetPositionY()));
        ...
        handler->PSendSysMessage("Hot Spots cercanos eliminados.");
        return true;
    }
};
...
public:
    ...
    void OnPlayerUpdate(Player* player, uint32 /*diff*/) override
    {
        ...
        const HotSpotData* spot = HotSpotMgr::instance()->GetHotSpotAt(player->GetMapId(), player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), 1);
        ...
        if (spot)
        {
            if (!player->HasAura(SPELL_HOTSPOT_XP))
            {
                ...
                ChatHandler(player->GetSession()).PSendSysMessage("¡Bono de Hot Spot activo! (Experiencia x{})", spot->xp_mult);
            }
        }
        else
        {
    ...
    void OnPlayerGiveXP(Player* player, uint32& amount, Unit* victim, uint8) override 
    {
        if (player->HasAura(SPELL_HOTSPOT_XP))
        {
            if (const HotSpotData* spot = HotSpotMgr::instance()->GetHotSpotAt(player->GetMapId(), player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), 1))
...
public:
    ...
    void OnUnitDeath(Unit* unit, Unit*) override 
    {
        ...
        if (const HotSpotData* spot = HotSpotMgr::instance()->GetHotSpotAt(creature->GetMapId(), creature->GetPositionX(), creature->GetPositionY(), creature->GetPositionZ(), 1))
        {
            // Respawn en 1 segundo
            creature->SetRespawnTime(1); 
            
            // El cuerpo desaparece en 3 segundos (estable y predecible)
